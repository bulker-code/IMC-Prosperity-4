...
class Trader:
    ...
    def micro_price(self, order_depth: OrderDepth):
        if not order_depth.buy_orders or not order_depth.sell_orders:
            return None
        best_bid = max(order_depth.buy_orders.keys())
        best_ask = min(order_depth.sell_orders.keys())
    ...
    def imbalance(self, order_depth: OrderDepth):
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            if product == "ASH_COATED_OSMIUM":
                ...
                mid_price = self.micro_price(order_depth)
                ...
                fair_value = np.mean(prices) + 0.5 * imb
                ...
                if len(order_depth.sell_orders) != 0:
                    best_ask, best_ask_amount = list(order_depth.sell_orders.items())[0]
                    buy_quantity = min(-best_ask_amount, max_position - position)
                    if int(best_ask) <  fair_value and z < 1:
                        print("BUY", str(-best_ask_amount), best_ask)
                        orders.append(Order(product, best_ask, buy_quantity))
                ...
                if len(order_depth.buy_orders) != 0:
                    best_bid, best_bid_amount = list(order_depth.buy_orders.items())[0]
                    sell_quantity = min(best_bid_amount, max_position + position)
                    if int(best_bid) > fair_value and z > 1:
                        print("SELL", str(sell_quantity), best_bid)
                        orders.append(Order(product, best_bid, -sell_quantity))

                if position > 0 and z > -0.5:
                    # close long early
                    best_bid = max(order_depth.buy_orders.keys())
